...
public class MsgWebSocketClient extends WebSocketClient {
    ...
    public void onMessage(String s) {
        ...
        try {
            if (s != null) {
                WebsocketData websocketData = gson.fromJson(s, WebsocketData.class);
                msgWebSocketClient_old.send(gson.toJson(websocketData));
            } else {
                LOGINFO.info("message from server is null");
            }
        } catch (Exception e) {
            e.printStackTrace();
